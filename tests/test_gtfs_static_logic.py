...
class GtfsStaticLogicTests(unittest.TestCase):
    def test_carregar_dados_estaticos_service_success(self):
        with tempfile.TemporaryDirectory() as tmp:
            ...
            os.chdir(tmp)
            try:
                with patch("gtfs_static_logic.requests.get", return_value=_FakeIbgeResponse()):
                    with patch("gtfs_static_logic.gpd.read_file", return_value=_FakeGaragensGdf()):
                        out = carregar_dados_estaticos_service(
                            empty_shapes_gdf_fn=lambda: pd.DataFrame(columns=["shape_id", "geometry"]),
                            empty_stops_gdf_fn=lambda: pd.DataFrame(columns=["stop_id", "stop_lat", "stop_lon"]),
                        )
            finally:
                os.chdir(old_cwd)
    ...
    def test_recarregar_gtfs_estatico_sob_demanda_service_empty_selection(self):
    ...
    def test_recarregar_gtfs_estatico_sob_demanda_service_without_routes_or_trips(self):
        with tempfile.TemporaryDirectory() as tmp:
    ...
    def test_carregar_dados_estaticos_service_stops_without_optional_columns(self):
        with tempfile.TemporaryDirectory() as tmp:
    ...
    def test_carregar_dados_estaticos_service_reuses_persistent_cache(self):
        with tempfile.TemporaryDirectory() as tmp:
            ...
            try:
                ...
                with patch("gtfs_static_logic.pd.read_csv", side_effect=AssertionError("cache nao foi utilizado")):
